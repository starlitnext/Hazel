...
public:
		...
		m_SquarePosition(0.0f)
	{
		...
		indexBuffer.reset(Hazel::IndexBuffer::Create(indicies, sizeof(vertices) / sizeof(uint32_t)));
	...
	{
		// HZ_INFO("Delta time: {0}s {1}ms", ts.GetSeconds(), ts.GetMilliseconds());
		...
		if (Hazel::Input::IsKeyPressed(HZ_KEY_A))
			m_CameraRotation += m_CameraRotattionSpeed * ts;
		...
		glm::mat4 transform = glm::translate(glm::mat4(1.0f), m_SquarePosition) * glm::scale(glm::mat4(1.0f), glm::vec3(1.5f));
		m_Texture->Bind();
...
private:
	...
	float m_CameraRotattionSpeed = 180.0f;
